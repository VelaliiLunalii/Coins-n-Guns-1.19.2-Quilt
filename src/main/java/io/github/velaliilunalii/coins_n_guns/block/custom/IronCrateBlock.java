...
public class IronCrateBlock extends Block implements ICrateBlock {
	private final Identifier tableToUse = new Identifier(CoinsNGuns.MOD_ID, "gameplay/iron_crate");
	...
	public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
		...
		if(world.isClient) {
			...
			} else if (mainHandStack.isOf(ModItems.CROWBAR)) {
				ItemStack itemStack = player.getStackInHand(hand);
				if (!player.getAbilities().creativeMode) {
					itemStack.decrement(1);
				}
				world.playSound((PlayerEntity) null, pos.getX(), pos.getY(), pos.getZ(), SoundEvents.BLOCK_METAL_BREAK, SoundCategory.NEUTRAL, 1F, 0.4F / (world.getRandom().nextFloat() * 0.4F + 0.8F));
				ItemEntity itemEntity = new ItemEntity(world, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(ModItems.GILDED_PISTOL_GRIP, 1));

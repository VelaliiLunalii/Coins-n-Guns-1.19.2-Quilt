...
public class GoldCoinProjectileEntity extends ThrownEntity {
	private float gravity = 0.04F;
	public GoldCoinProjectileEntity(LivingEntity shooter, World world) {
		super(ModEntities.GOLD_COIN_PROJECTILE, shooter, world);
	}

	public float getSpeed(){return 2.8F;}
	@Override
	...
	public GoldCoinProjectileEntity(EntityType<GoldCoinProjectileEntity> entityType, World world) {
		super(entityType, world);
	}

	protected GoldCoinProjectileEntity(EntityType<? extends ThrownEntity> type, double x, double y, double z, World world) {
	...
	protected void onEntityHit(EntityHitResult entityHitResult) {
		...
		entity.damage(DamageSource.thrownProjectile(this, this.getOwner()), (float) this.getDamage() * (1+((float) getRicochet())));
		...
		if(getPouchType().equalsIgnoreCase("Sculk") && !world.isClient) {
			...
			List<GoldCoinProjectileEntity> coinProjectileEntityList = world.getEntitiesByClass(
				GoldCoinProjectileEntity.class,
				box,
				Entity -> !Entity.equals(this) && Entity.getOwner() == this.getOwner()
			);

			for (GoldCoinProjectileEntity coin : coinProjectileEntityList) {
	...
	protected void onBlockHit(BlockHitResult blockHitResult) {
		...
		Vec3d newVelocity = calculateBounceVelocity(velocity, hitDirection);

		this.setVelocity(newVelocity.multiply(0.5, 0.5, 0.5));
	...
	private Vec3d calculateBounceVelocity(Vec3d velocity, Direction hitDirection) {
		if(hitDirection == Direction.DOWN || hitDirection == Direction.UP) return new Vec3d(velocity.x, -velocity.y, velocity.z);
		if(hitDirection == Direction.NORTH || hitDirection == Direction.SOUTH) return new Vec3d(velocity.x, velocity.y, -velocity.z);
		if(hitDirection == Direction.EAST || hitDirection == Direction.WEST) return new Vec3d(-velocity.x, velocity.y, velocity.z);
		return velocity;
	}
	...
	protected void onCollision(HitResult hitResult) {
		...
		if (!this.world.isClient) {
			if ((type == HitResult.Type.ENTITY && getPierce() >= max_pierce) || (type == HitResult.Type.BLOCK && getRicochet() >= max_ricochet)) {
		...
		if(world instanceof ServerWorld serverWorld){
			...
			serverWorld.spawnParticles(ModParticles.GOLD_COIN_IMPACT_PARTICLE, hit_pos.x, hit_pos.y, hit_pos.z, 1, 0, 0, 0, 0);
		}
	}

	private final int max_ricochet = 1;
	...
	private static final TrackedData<Boolean> MULTISHOT_COPY = DataTracker.registerData(GoldCoinProjectileEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
	private static final TrackedData<Integer> PIERCE = DataTracker.registerData(GoldCoinProjectileEntity.class, TrackedDataHandlerRegistry.INTEGER);
	private static final TrackedData<Integer> RICOCHET = DataTracker.registerData(GoldCoinProjectileEntity.class, TrackedDataHandlerRegistry.INTEGER);
	private static final TrackedData<String> POUCH_TYPE = DataTracker.registerData(GoldCoinProjectileEntity.class, TrackedDataHandlerRegistry.STRING);
	private static final TrackedData<Boolean> MAGNETIC_FIELD_INCREASED = DataTracker.registerData(GoldCoinProjectileEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
	@Override
	...
	protected void writeCustomDataToNbt(NbtCompound nbt) {
		...
		nbt.putInt("Ricochet", this.dataTracker.get(RICOCHET));
	...
	protected void readCustomDataFromNbt(NbtCompound nbt) {
		...
		this.dataTracker.set(RICOCHET, nbt.getInt("Ricochet"));
	...
	public boolean getMultishotCopy() {
		return this.dataTracker.get(MULTISHOT_COPY);
	}

	public void setMultishotCopy(boolean multishotCopy) {
		this.dataTracker.set(MULTISHOT_COPY, multishotCopy);
	}
	...
	public int getRicochet() {
		return this.dataTracker.get(RICOCHET);
	}

	public void setRicochet(int ricochet) {
		this.dataTracker.set(RICOCHET, ricochet);
	}
